...
inference_layer = tf.keras.layers.TFSMLayer("model/saved_model", call_endpoint="serving_default")

input_shape = (128, 128, 3)

# Wrap it in a Keras model
model = tf.keras.Sequential([
    tf.keras.Input(shape=input_shape),  # Define appropriate input shape
    inference_layer
])
...
def predict():
    ...
    if request.method == "POST":
        ...
        binary_predictions = (predictions['dense_21'][0][0] > 0.5).astype(int)
